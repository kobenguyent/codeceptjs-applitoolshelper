...
class ApplitoolsHelper extends Helper {
    ...
    constructor(config: ApplitoolsHelperConfig) {
        ...
        if (!config.applitoolsKey) throw Error('Please check your Applitools API key!');
    ...
    async _before() {
        ...
        client = this['helpers'][_helper].page ? this['helpers'][_helper].page : this['helpers'][_helper].browser;
    }
     ...
     *
     */
    async eyeCheck({ pageName, element, uniqueId, matchLevel }:{pageName: string, element?: string, uniqueId?: string, matchLevel?: MatchLevel}): Promise<void> {
